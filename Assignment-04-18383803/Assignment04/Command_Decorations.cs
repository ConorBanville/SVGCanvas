...
{
    /*  This is the command used to update the style attributes of a shape
     *  
     ...
     * 
     */
    class Command_Decorations : ICommand
    {
        string id_of_shape;
        string[] data;
        string[] old_data;

        public Command_Decorations(string id_of_shape, string[] data)
        {
        ...
        void ICommand.execute()
        {
            try
            {
                old_data = Program.canvas.Decorations(Convert.ToInt32(id_of_shape),data);
            }
            catch
            {
                old_data = null;
        ...
        void ICommand.unexecute()
        {
            ...
            else
            {
                try
                {
                    Program.canvas.Decorations(Convert.ToInt32(id_of_shape), old_data);
                }
                catch
                {
                    return;
